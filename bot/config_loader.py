...
class ConfigLoader:
    ...
    Cache state is intentionally per-instance (not class-level) so that
    multiple ConfigLoader instances — e.g. in tests — do not share state.
    The Function App creates a single module-level instance via _get_handler(),
    so in production there is effectively one cache per warm instance.
    """
    ...
    def __init__(self):
        ...
        # Per-instance cache — NOT class-level
        self._cache: dict = {}
