...
class NameMatcher:
    ...
    def _fuzzy(self, spoken: str, staff: list) -> MatchResult:
        ...
        Note on indexing: process.extract returns (matched_string, score, index)
        tuples where the index is the position in the `names` list passed to it.
        Because `candidates` and `names` are built with identical ordering via
        the same list comprehension, candidates[best_idx] correctly resolves
        the winning StaffMember without any secondary lookup.
        """
        candidates: list = []
        for member in staff:
            for token in member.searchable_tokens:
